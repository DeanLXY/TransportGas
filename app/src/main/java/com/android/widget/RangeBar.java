...
public class RangeBar extends View {
	...
	private static final int DEFAULT_CONNECTING_LINE_COLOR = 0xffffc90b;
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		mLeftThumb = new Thumb(ctx, yPos, mThumbColorNormal, mThumbColorPressed, mThumbRadiusDP, mThumbImageNormal,
				mThumbImagePressed);
		mRightThumb = new Thumb(ctx, yPos, mThumbColorNormal, mThumbColorPressed, mThumbRadiusDP, mThumbImageNormal,
				mThumbImagePressed);
	...
	private void rangeBarInit(Context context, AttributeSet attrs) {
		...
		try {
			...
			mThumbImageNormal = ta.getResourceId(R.styleable.RangeBar_thumbImageNormal, DEFAULT_THUMB_IMAGE_NORMAL);
	...
	private void createBar() {
		...
		mBar = new Bar(getContext(), getMarginLeft(), getYPos(), getBarLength(), mTickCount, mTickHeightDP, mBarWeight,
				mBarColor);
		invalidate();
	}
	...
	private void onActionDown(float x, float y) {
		...
		//
		// } else if (!mLeftThumb.isPressed() && mRightThumb.isInTargetZone(x,
		// y)) {
	 ...
	 * 
	 * @param x
	 *            the x-coordinate of the up action
	 * @param y
	...
	private void onActionUp(float x, float y) {
		...
		if (mLeftThumb.isPressed()) {
			...
			// If either of the indices have changed, update and call the
			// listener.
			if (newLeftIndex != mLeftIndex || newRightIndex != mRightIndex) {
	...
	public void setRange(int x){
		final float nearestTickX = mBar.getNearestTickCoordinate(x);
		mRightThumb.setX(nearestTickX);
		mRightThumb.release();
		invalidate();
	}
	/**
